...
export default function NewRoutinePage() {
    ...
    const handleExerciseChange = (index: number, field: keyof CreateRoutineExercise, value: string | number) => {
    ...
    const addExercise = () => {
        const nextOrder = exercises.length > 0 ? exercises[exercises.length - 1].order + 1 : 1;
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => handleExerciseChange(index, 'exercise_api_id', e.target.value)}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                    required
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Sets (optional)</label>
                                <Input
                                    type="number"
                                    value={exercise.sets || ''}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Reps (optional)</label>
                                <Input
                                    type="number"
                                    value={exercise.reps || ''}
